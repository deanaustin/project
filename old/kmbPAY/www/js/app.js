...
app.run(function(SettleFactory, ChangeIdFactory, ClientTokenFactory, Hour, NavigateFactory, Time, OrderList, SupportFactory, TimeFactory, StockFactory, CustomFactory, OrderFactory, UserFactory, AuthFactory, SupportFactory, Fire, $interval, $window, $ionicPlatform, $state,  $rootScope, $timeout) {
  ...
  $interval(function(){
    ...
    var auth = null;
      ...
      $rootScope.done = function(){
        $rootScope.productName.name = 'test';
        CustomFactory.add($rootScope.stockCus, $rootScope.productName.name, $rootScope.authData.uid, function(ref){
         
        });
         OrderFactory.add($rootScope.stockCus, $rootScope.productName.name, $rootScope.authData.uid);
          $rootScope.productName.name = '';
          $state.go("app.main");
      };
      ...
      $rootScope.setUpPayment = function(){
          ...
          ClientTokenFactory.init(authData, function(clientToken){
      ...
      $rootScope.submitOrder = function(nonce){
        ...
        $rootScope.newOrder = {};
        OrderFactory.submit(nonce, orderCopy)
        OrderFactory.init(authData, function(order){
          $rootScope.newOrder = order;
        });
        $state.go("app.main"); 
     ...
     $rootScope.userInfoReg = {email: '', password: ''};
    $rootScope.register = function(email, password) {
        $rootScope.auth.$createUser({email: $rootScope.userInfoReg.email, password: $rootScope.userInfoReg.password}).then(function(userData) {
            
            var user = UserFactory.user(userData);
            user.authType = 'cus';
            user.$save();
            if(!$rootScope.authData.password){
              ChangeIdFactory.change(userData.uid, $rootScope.authData.uid);
              CustomFactory.modify(userData.uid);
              OrderFactory.modify(userData.uid);
            }
            
            
            $rootScope.auth.$authWithPassword({email: $rootScope.userInfoReg.email, password: $rootScope.userInfoReg.password}).then(function(authData) {
              $rootScope.userInfoReg.email = '';
              $rootScope.userInfoReg.password = '';
  ...
  var obj = UserFactory.user(authData);
            obj.$loaded(function(data){
              if(data.authType === 'admin'){
                ...
                var someList = CustomFactory.custom(authData);
                someList.$loaded(function(data){
...
app.factory('SettleFactory', function(Fire){
  var self = this;
  self.settle = function(key, transactionId){
    console.log('order id: ' + key);
    Fire.ref.child("settle").push({transactionId: transactionId, orderId: key});
  }
  return self;
});

app.factory('ChangeIdFactory', function(Fire){
  var self = this;
  self.change = function(newId, oldId){
    Fire.ref.child("changeId").push({newId, oldId});
  };
  return self;
});

app.factory('ClientTokenFactory', function(Fire, $firebaseObject){
  var self = this;
  self.obj = null;
  self.init = function(authData, callback){
    var push = Fire.ref.child("clientToken").push();
    var key = push.key()
    push.set({"token": "NA", "customerId": authData.uid}, function(){
         
    });
    Fire.ref.child("clientToken").child(key).on('value', function(dataSnapshot){
      if(dataSnapshot.val().token != 'NA'){
        Fire.ref.child("clientToken").child(key).off();
        Fire.ref.child("clientToken").child(key).remove();
        self.obj = dataSnapshot.val();
        callback(self.obj);
