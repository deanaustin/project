...
app.run(function($http, Hour, NavigateFactory, Time, OrderList, SupportFactory, TimeFactory, StockFactory, CustomFactory, OrderFactory, UserFactory, AuthFactory, SupportFactory, Fire, $interval, $window, $ionicPlatform, $state,  $rootScope, $timeout) {
      ...
      $rootScope.verifyProfile = function(){
        if($rootScope.newOrder.reference == null || $rootScope.newOrder.reference == ""
          ...
          if((hour < Hour.obj.open || hour >= Hour.obj.close) && OrderFactory.order.ms === 9999999999999){
            ...
            $state.go("app.main.confirm.confirmsubmit");
          }
    ...
    StockFactory.addItem(item, option, function(data){
      $rootScope.item = data;
        ...
        $rootScope.login = function(){
          $rootScope.auth.$authWithPassword({email: $rootScope.userInfo.phone + "@foobar.com.au", password: $rootScope.userInfo.phone}).then(function(authData) {
          }).catch(function(error) {});
        };
...
app.config(function($stateProvider, $urlRouterProvider) {
  ...
  .state('app.main.confirm.method', {
    url: '/method',
    views: {
      'menuContent@app': {
        templateUrl: 'templates/pay/method.html'
      }
    }
  })
  
  .state('app.main.confirm.method.pay', {
    url: '/pay',
    views: {
      'menuContent@app': {
        templateUrl: 'templates/pay/pay.html',
        controller: function($rootScope, $http, $timeout){
          $http.get('https://blah2-deanaustin.c9users.io/client_token', {params:{customerId: '123456'}}).then(function(response){
            var clientToken = response.data;
            braintree.setup(clientToken, "dropin", {
              container: "payment-form",
              paymentMethodNonceReceived: function (event, nonce) {
                $timeout(function(){
                  $http.post('https://blah2-deanaustin.c9users.io/checkout', {
...
app.factory('OrderFactory', function(Fire, $firebaseArray, $firebaseObject, UserFactory, CustomFactory){
  var self = this;
  self.order = {};
  self.init = function(authData, callback){
    ...
    user.$loaded(function(data){
      ...
      list.$loaded(function(x){
          ...
          self.order = {method: 'In-store', total: 0, ms: 9999999999999, mobile: authData.password.email, order: 0, reference: reference , done: "Order", user: authData.uid, customList: angular.copy(x)};
  ...
  self.submit = function(orderCopy){
    ...
    order.$add(orderCopy).then(function(){});
  }
  ...
  self.userOrders = function(authData){
    return $firebaseArray(Fire.ref.child("order").orderByChild("user").equalTo(authData.uid));
  };
...
app.factory('CustomFactory', function(Fire, $firebaseArray, $firebaseObject){
  ...
  self.custom = function(authData){
    return $firebaseArray(Fire.ref.child("custom").orderByChild("user").equalTo(authData.uid));
  }
...
app.factory('StockFactory', function(Fire, $firebaseArray, $firebaseObject){
  var self = this;
  self.stock = null;
  self.item = null;
